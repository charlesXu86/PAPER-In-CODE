...
class LSTMParser(object):
    ...
    def __init__(self, 
        ...
        self._NT_REL = get_nt_rel(self.act_vocab)
        self._AND = get_and(self.act_vocab)
        self._COUNT = get_count(self.act_vocab)  
        self._AGGREGATION = get_aggregation(self.act_vocab)
        self._TER_REL = get_ter_rel(self.act_vocab)
        self._TER_ENT = get_ter_ent(self.act_vocab)
        if order == 'top_down':
    ...
    def train(self, words, oracle_actions, oracle_tokens, ent, rel, options, kb):
        '''
        ...
        if self.order == 'top_down':
            loss = self.top_down_train(words, ent, rel, oracle_actions, oracle_tokens, options, buffer, stack_top, action_top, kb)
        elif self.order == 'bottom_up':
            loss = self.bottom_up_train(words, ent, rel, oracle_actions, oracle_tokens, options, buffer, stack_top, action_top, kb)
        else:
    ...
    def parse(self, words, ent, rel, kb):
        '''
        ...
        if self.order == 'top_down':
            if not self.beam_search:
                output_actions, output_tokens = self.top_down_parse(words, ent, rel, buffer, stack_top, action_top, kb)
            else:
                output_actions, output_tokens, beam = self.top_down_beam_search(words, ent, rel, buffer, stack_top, action_top, kb, self.beam_size)
        elif self.order == 'bottom_up':
            if not self.beam_search:
                output_actions, output_tokens = self.bottom_up_parse(words, ent, rel, buffer, stack_top, action_top, kb)
            else:
                output_actions, output_tokens, beam = self.bottom_up_beam_search(words, ent, rel, buffer, stack_top, action_top, kb, self.beam_size)
        else:
            raise NotImplementedError("Generation order not supported")
        ...
        if not self.beam_search:
            ...
            return output_actions, output_tokens, beam

    def top_down_train(self, words, ent, rel, oracle_actions, oracle_tokens, options, buffer, stack_top, action_top, kb):
        stack = []
        losses = []
        stack_token = []
        nt_rel = [self.nt_vocab[t] for t in rel if self.nt_vocab.has_key(t)]
        ter_rel = [self.ter_vocab[t] for t in rel if self.ter_vocab.has_key(t)]
        ter_ent = [self.ter_vocab[t] for t in ent if self.ter_vocab.has_key(t)]
        ...
        while not (len(stack) == 1 and reduced != 0):
            ...
            if action == self._NT_REL:
                #generate non-terminal
                parent_rel = oracle_tokens.pop(0)
                nt = self.nt_vocab[parent_rel]
                assert (nt in nt_rel)
                output_feature, output_logprob = attention_output(buffer, word_weights, "soft_average")
                log_probs_nt = dy.log_softmax(self.nt_proj_layer(output_feature), restrict=nt_rel)
            ...
            elif action == self._TER_REL:
                #generate terminal
                ter = self.ter_vocab[oracle_tokens.pop(0)]
                assert (ter in ter_rel)
                output_feature, output_logprob = attention_output(buffer, word_weights, "soft_average") 
                log_probs_ter = dy.log_softmax(self.ter_proj_layer(output_feature), restrict=ter_rel)
            ...
            elif action == self._TER_ENT: 
                ter = self.ter_vocab[oracle_tokens.pop(0)]
                assert (ter in ter_ent)
                output_feature, output_logprob = attention_output(buffer, word_weights, "soft_average")
                res_ent = kb.head_ent_lookup(parent_rel)
                if len(res_ent) > 0:
                    ent = res_ent
                log_probs_ter = dy.log_softmax(self.ter_proj_layer(output_feature), restrict=ter_ent)
                losses.append(-dy.pick(log_probs_ter, ter))
    ...
    def bottom_up_train(self, words, ent, rel, oracle_actions, oracle_tokens, options, buffer, stack_top, action_top, kb):
        stack = []
        losses = []
        stack_token = []
        past_actions = []
        ...
        while not (found_root):
            valid_actions = get_valid_actions_bu(stack_token, self.act_vocab, past_actions) 
            action = self.act_vocab[oracle_actions.pop(0)]
            ...
            if action == self._NT_REL:
                #label bottom_up
                nt = self.nt_vocab[oracle_tokens.pop(0)]
                rel = kb.relation_lookup(child_ent)
                nt_rel = [self.nt_vocab[t] for t in rel if self.nt_vocab.has_key(t)]
                assert (nt in nt_rel)
                output_feature, output_logprob = attention_output(buffer, word_weights, 'soft_average')
                log_probs_nt = dy.log_softmax(self.nt_proj_layer(output_feature), restrict=nt_rel)
                ...
                child_ent = oracle_tokens.pop(0)
                ter = self.ter_vocab[child_ent]
                assert (ter in ter_ent)
                output_feature, output_logprob = attention_output(buffer, word_weights, 'soft_average') 
                log_probs_ter = dy.log_softmax(self.ter_proj_layer(output_feature), restrict=ter_ent)
                losses.append(-dy.pick(log_probs_ter, ter))
            ...
            action_embedding = self.act_input_layer(self.act_lookup[action])
            action_top = action_top.add_input(action_embedding)
            past_actions.append(self.act_vocab.token(action))
        return dy.esum(losses)


    def top_down_parse(self, words, ent, rel, buffer, stack_top, action_top, kb):
        stack = []
        stack_token = []
        nt_rel = [self.nt_vocab[t] for t in rel if self.nt_vocab.has_key(t)]
        ter_rel = [self.ter_vocab[t] for t in rel if self.ter_vocab.has_key(t)]
    ...
    def top_down_beam_search(self, words, ent, rel, buffer, stack_top, action_top, kb, beam_size=5):
        finished = []    
        stack = []
        nt_rel = [self.nt_vocab[t] for t in rel if self.nt_vocab.has_key(t)]
        ter_rel = [self.ter_vocab[t] for t in rel if self.ter_vocab.has_key(t)]
            ...
            for der in beam:
                ...
                for action, logprob in zip(sorted_actions, sorted_logprobs):
                    if action == self._NT_REL:
                        ...
                        output_feature, output_logprob = attention_output(buffer, word_weights, self.test_selection, argmax=True)
                        parent_rel = der.output_tokens[-1]
                        res_ent = kb.head_ent_lookup(parent_rel)
                        if len(res_ent) > 0:
                            ent = res_ent
                        ter_ent = [self.ter_vocab[t] for t in ent if self.ter_vocab.has_key(t)]
        ...
        return finished[0].output_actions, finished[0].output_tokens, finished


    def bottom_up_parse(self, words, ent, rel, buffer, stack_top, action_top, kb):
        stack = []
        stack_token = []
        output_actions = []
        output_tokens = []
        ...
        #recursively generate the tree until training data is exhausted
        while not (found_root):
            valid_actions = get_valid_actions_bu(stack_token, self.act_vocab, output_actions) 
    ...
    def bottom_up_beam_search(self, words, ent, rel, buffer, stack_top, action_top, kb, beam_size=5):
        finished = []    
        stack = []
        stack_token = []
        nt_rel = [self.nt_vocab[t] for t in rel if self.nt_vocab.has_key(t)]
        ...
        action_summary = self.initial_embedding()
        word_weights = self.attention(stack_embedding, buffer)
        buffer_embedding, _ = attention_output(buffer, word_weights, 'soft_average')
        ...
        for action, logprob in zip(sorted_actions, sorted_logprobs):
            ...
            if action == self._TER_REL:
                ...
                for i in range(beam_size):
                     ...
                     beam.append(Derivation([(stack_state, 'c', ter_embedding)], action_top, [self.act_vocab.token(action)], [self.ter_vocab.token(ter)],
                                                 logprob+sorted_ter_logprobs[i][1], [self.ter_vocab.token(ter)]))
        ...
        beam.sort(key=lambda x: x.logp, reverse=True)   
        ...
        while len(finished) < beam_size:
            ...
            # collect all possible expanded beam
            for der in beam:
                valid_actions = get_valid_actions_bu(der.stack_token, self.act_vocab, der.output_actions) 
